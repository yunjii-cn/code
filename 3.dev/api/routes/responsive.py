...
async def scan(req: ScanRequest, workspace_path: Optional[str] = Query(None)):
    ...
    # 2026-06-09 TASK-4.3 引入：自动推送到 Web Push 订阅（仅 warning/error）
    if payload:
        _maybe_push_notifications(payload, workspace_path)
...
# ──────────── 2026-06-09 TASK-4.3 引入：Web Push 集成 ────────────


def _maybe_push_notifications(notifs: List[Notification], workspace_path: Optional[str]) -> None:
    """对 warning/error 级别的通知触发 Web Push

    - 失败时静默（不影响主流程）
    - 同步调用（pywebpush 是同步的）
    - 节流 5 分钟：同 type+id 不重复推
    """
    try:
        ...
        for n in notifs:
            data = _serial(n)
            if not should_push(data, "warning"):
                continue
            payload = build_push_payload_from_notification(data, workspace_path or "")
            throttle_tag = f"ws:{n.type.value}:{n.id[:8]}"
            try:
                sender.send_to_all(payload, throttle_tag=throttle_tag, throttle_seconds=300)
            except Exception as e:
                logger.debug(f"[Responsive] push failed for {n.id}: {e}")
    except Exception as e:
        logger.debug(f"[Responsive] push module unavailable: {e}")
